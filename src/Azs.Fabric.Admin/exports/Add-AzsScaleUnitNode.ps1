...
begin {
    try {
        ...
        $mapping = @{
            ScaleExpanded = 'Azs.Fabric.Admin.custom\Add-AzsScaleUnitNode';
            Scale = 'Azs.Fabric.Admin.custom\Add-AzsScaleUnitNode';
            ScaleViaIdentityExpanded = 'Azs.Fabric.Admin.custom\Add-AzsScaleUnitNode';
            ScaleViaIdentity = 'Azs.Fabric.Admin.custom\Add-AzsScaleUnitNode';
        }
        if (('ScaleExpanded', 'Scale') -contains $parameterSet -and -not $PSBoundParameters.ContainsKey('Location')) {
            $PSBoundParameters['Location'] = (Get-AzLocation)[0].Location
        }
        if (('ScaleExpanded', 'Scale') -contains $parameterSet -and -not $PSBoundParameters.ContainsKey('ResourceGroupName')) {
            $PSBoundParameters['ResourceGroupName'] = -join("System.",(Get-AzLocation)[0].Location)
        }
        if (('ScaleExpanded', 'Scale') -contains $parameterSet -and -not $PSBoundParameters.ContainsKey('SubscriptionId')) {
