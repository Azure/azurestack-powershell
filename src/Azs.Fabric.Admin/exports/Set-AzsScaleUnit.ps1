...
begin {
    try {
        ...
        $mapping = @{
            SetExpanded = 'Azs.Fabric.Admin.custom\Set-AzsScaleUnit';
        }
        if (('SetExpanded') -contains $parameterSet -and -not $PSBoundParameters.ContainsKey('Location')) {
            $PSBoundParameters['Location'] = (Get-AzLocation)[0].Location
        }
        if (('SetExpanded') -contains $parameterSet -and -not $PSBoundParameters.ContainsKey('ResourceGroupName')) {
