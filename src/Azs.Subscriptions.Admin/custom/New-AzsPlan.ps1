...
process {
    if ($PSCmdlet.ParameterSetName -eq 'CreateExpanded') {
        ...
        $resource = Get-AzsPlan @params -ErrorAction SilentlyContinue
        if ($null -ne $resource) { throw "$($MyInvocation.MyCommand): A plan with name $Name in resource group $ResourceGroupName already exists" }
