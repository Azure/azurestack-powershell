...
process {
    if ($PSCmdlet.ParameterSetName -eq 'CreateExpanded') {
        ...
        $params = @{}
        $relevantParams = @('Name', 'ResourceGroupName', 'SubscriptionId', 'DefaultProfile', 'Break', 
                            'HttpPipelineAppend', 'HttpPipelinePrepend', 'Proxy', 'ProxyCredential', 
                            'ProxyUseDefaultCredentials')
        foreach ($p in $relevantParams) {
            if ($PSBoundParameters.ContainsKey($p)) { $params[$p] = $PSBoundParameters[$p] }
        }
        $resource = Get-AzsAdminManagedOffer @params -ErrorAction SilentlyContinue
        if ($null -ne $resource) { throw "$($MyInvocation.MyCommand): An offer with name $Name in resource group $ResourceGroupName already exists" }
        # Name, PropertiesName and DisplayName are duplicate parameters generated by autorest. 
        ...
        $PSBoundParameters['PropertiesName'] = $Name
        if (-not $PSBoundParameters.ContainsKey(('DisplayName'))) { $PSBoundParameters['DisplayName'] = $Name }
    }
