...
begin {
    try {
        ...
        $parameterSet = $PSCmdlet.ParameterSetName
        $mapping = @{
            List = 'Azs.Update.Admin.custom\Get-AzsUpdate';
            Get = 'Azs.Update.Admin.custom\Get-AzsUpdate';
            GetViaIdentity = 'Azs.Update.Admin.custom\Get-AzsUpdate';
        }
...
begin {
    try {
        ...
        $parameterSet = $PSCmdlet.ParameterSetName
        $mapping = @{
            List = 'Azs.Update.Admin.custom\Get-AzsUpdateRun';
            Get = 'Azs.Update.Admin.custom\Get-AzsUpdateRun';
            GetViaIdentity = 'Azs.Update.Admin.custom\Get-AzsUpdateRun';
        }
        if (('List', 'Get') -contains $parameterSet -and -not $PSBoundParameters.ContainsKey('Location')) {
            $PSBoundParameters['Location'] = (Get-AzLocation)[0].Location
        }
...
[CmdletBinding(DefaultParameterSetName='Rerun', PositionalBinding=$false)]
param(
...
begin {
    try {
        ...
        $parameterSet = $PSCmdlet.ParameterSetName
        $mapping = @{
            Rerun = 'Azs.Update.Admin.custom\Resume-AzsUpdateRun';
            RerunViaIdentity = 'Azs.Update.Admin.custom\Resume-AzsUpdateRun';
        }
        if (('Rerun') -contains $parameterSet -and -not $PSBoundParameters.ContainsKey('Location')) {
            $PSBoundParameters['Location'] = (Get-AzLocation)[0].Location
        }
        if (('Rerun') -contains $parameterSet -and -not $PSBoundParameters.ContainsKey('ResourceGroupName')) {
            $PSBoundParameters['ResourceGroupName'] = -join("System.",(Get-AzLocation)[0].Location)
        }
        if (('Rerun') -contains $parameterSet -and -not $PSBoundParameters.ContainsKey('SubscriptionId')) {
            $PSBoundParameters['SubscriptionId'] = (Get-AzContext).Subscription.Id
        }
        $cmdInfo = Get-Command -Name $mapping[$parameterSet]
        [Microsoft.Azure.PowerShell.Cmdlets.UpdateAdmin.Runtime.MessageAttributeHelper]::ProcessCustomAttributesAtRuntime($cmdInfo, $MyInvocation, $parameterSet, $PSCmdlet)
