...
begin {
    try {
        ...
        $mapping = @{
            Close = 'Azs.Infrastructureinsights.Admin.private\Close-AzsAlert_Close';
            CloseExpanded = 'Azs.Infrastructureinsights.Admin.private\Close-AzsAlert_CloseExpanded';
            CloseViaIdentity = 'Azs.Infrastructureinsights.Admin.private\Close-AzsAlert_CloseViaIdentity';
            CloseViaIdentityExpanded = 'Azs.Infrastructureinsights.Admin.private\Close-AzsAlert_CloseViaIdentityExpanded';
        }
        if (('Close', 'CloseExpanded', 'CloseViaIdentityExpanded') -contains $parameterSet -and -not $PSBoundParameters.ContainsKey('Location')) {
            $PSBoundParameters['Location'] = (Get-AzLocation)[0].Location
        }
        if (('Close', 'CloseExpanded') -contains $parameterSet -and -not $PSBoundParameters.ContainsKey('ResourceGroupName')) {
            $PSBoundParameters['ResourceGroupName'] = -join("System.",(Get-AzLocation)[0].Location)
        }
        if (('Close', 'CloseExpanded') -contains $parameterSet -and -not $PSBoundParameters.ContainsKey('SubscriptionId')) {
