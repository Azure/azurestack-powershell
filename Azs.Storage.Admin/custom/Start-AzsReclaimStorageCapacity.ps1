...
Start reclaim storage capacity on deleted storage objects.
.Example
To view examples, please use the -Online parameter with Get-Help or navigate to: https://docs.microsoft.com/en-us/powershell/module/azs.storage.admin/start-azsreclaimstoragecapacity
.Outputs
System.Boolean
.Link
...
process {
    if ($PSCmdlet.ShouldProcess("Storage Accounts" , "Start reclaim storage capacity"))
    {
        if ($Force.IsPresent -or $PSCmdlet.ShouldContinue("Start reclaim storage capacity?", "Performing operation garbage collect for deleted storage accounts."))
        {
            ...
            Azs.Storage.Admin.internal\Start-AzsReclaimStorageCapacity @PSBoundParameters
        }
    }
}
}
